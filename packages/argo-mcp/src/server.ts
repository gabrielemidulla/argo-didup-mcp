...
function authorize(req: Request): boolean {
  const expected = mcpServerEnv.AUTH_TOKEN?.trim();
...
export async function startHttpServer() {
  const port = Number(mcpServerEnv.PORT);
  const tokenConfigured = Boolean(mcpServerEnv.AUTH_TOKEN?.trim());
  const explicitHost = mcpServerEnv.MCP_HOST?.trim();
  const hostname =
  ...
  const rawIdle = mcpServerEnv.MCP_IDLE_TIMEOUT;
  const idleTimeout =
