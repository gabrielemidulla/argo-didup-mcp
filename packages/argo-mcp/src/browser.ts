...
// Backbase/Argo gate on `bb` and automation flags; without this, headless login and navigation often break.
...
export function getArgoCredentialsFromEnv(): ArgoCredentials | null {
  const codiceScuola = mcpServerEnv.CODICE_SCUOLA;
  const username = mcpServerEnv.USERNAME;
  const password = mcpServerEnv.PASSWORD;
